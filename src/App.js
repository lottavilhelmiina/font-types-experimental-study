...
function App() {
  ...
  const [textTable, setTextTable] = useState(DATA);
  ...
  const selectedTable = textTable.find(t => t.id === tableID);
  ...
  const addNote = () => {
    ...
    if (tableID === 0) {
      const practiceObject = {
        ...
        participantID: 1,
        question1: selectedTable.description1,
        answer1: answer1
      }
      axios
        ...
        backupTable.push(practiceObject);
      setAnswer1("");
      ...
      const noteObject = {
        ...
        question1: selectedTable.description1,
        answer1: answer1,
        question2: selectedTable.description2,
        answer2: answer2,
        question3: selectedTable.description3,
        answer3: answer3,
        question4: selectedTable.description4,
        answer4: answer4,
        question5: selectedTable.description5,
        answer5: answer5,
        question6: selectedTable.description6,
        answer6: answer6
      }
      axios
...
<Toolbar />
      ...
      <Typography sx={{ textAlign: 'left', fontFamily: selectedTable.font, fontSize: '15px', color: '#1A1A1A', marginBottom: '20px' }}>Each test has a time limit of 10 minutes. After this, the program automatically moves to the next phase. If you are ready before the time expires, you can press the button on the bottom of the page to move forward. Do not press the button before finishing your answers, nor refresh the page at any point!</Typography>
        <Typography sx={{ textAlign: 'left', fontFamily: selectedTable.font, fontSize: '15px', color: '#1A1A1A', marginBottom: '80px' }}>Answer to each text field with a plain alphabet letter from A to D, accordingly to the questions. If you do not know the answer, do not guess, instead leave the answer blank.</Typography>

{tableID < 1 && <Typography sx={{ textAlign: 'left', fontFamily: selectedTable.font, fontSize: '24px', fontWeight: 'bold', color: '#1A1A1A', marginBottom: '20px' }}>Practice test</Typography>}
{tableID > 0 && tableID < 4 && <Typography sx={{ textAlign: 'left', fontFamily: selectedTable.font, fontSize: '24px', fontWeight: 'bold', color: '#1A1A1A', marginBottom: '20px' }}>Test {tableID}</Typography>}
            <Typography sx={{ fontFamily: selectedTable.font, margin: '15px 0px', fontSize: '20px', fontWeight: 'bold', color: '#1A1A1A' }}>{selectedTable.header}</Typography>
            <Typography sx={{ fontFamily: selectedTable.font, margin: '15px 0px', color: '#1A1A1A' }}>{selectedTable.pg1}</Typography>
            <Typography sx={{ fontFamily: selectedTable.font, margin: '15px 0px', color: '#1A1A1A' }}>{selectedTable.pg2}</Typography>
            <Typography sx={{ fontFamily: selectedTable.font, margin: '15px 0px', color: '#1A1A1A' }}>{selectedTable.pg3}</Typography>
            <Typography sx={{ fontFamily: selectedTable.font, margin: '15px 0px', color: '#1A1A1A' }}>{selectedTable.pg4}</Typography>
            <Typography sx={{ fontFamily: selectedTable.font, margin: '15px 0px', color: '#1A1A1A' }}>{selectedTable.pg5}</Typography>
            <Typography sx={{ fontFamily: selectedTable.font, margin: '15px 0px', color: '#1A1A1A' }}>{selectedTable.pg6}</Typography>
            <Typography sx={{ fontFamily: selectedTable.font, margin: '15px 0px', color: '#1A1A1A' }}>{selectedTable.pg7}</Typography>
            <Typography sx={{ fontFamily: selectedTable.font, margin: '15px 0px', color: '#1A1A1A' }}>{selectedTable.pg8}</Typography>
            <Typography sx={{ fontFamily: selectedTable.font, margin: '15px 0px', color: '#1A1A1A' }}>{selectedTable.pg9}</Typography>
            <Typography sx={{ fontFamily: selectedTable.font, margin: '15px 0px', color: '#1A1A1A' }}>{selectedTable.pg10}</Typography>
            <Typography sx={{ fontFamily: selectedTable.font, margin: '15px 0px', color: '#1A1A1A' }}>{selectedTable.pg11}</Typography>
            <Typography sx={{ fontFamily: selectedTable.font, margin: '15px 0px', color: '#1A1A1A' }}>{selectedTable.pg12}</Typography>
            <Typography sx={{ fontFamily: selectedTable.font, margin: '15px 0px', color: '#1A1A1A' }}>{selectedTable.pg13}</Typography>
            <Typography sx={{ fontFamily: selectedTable.font, margin: '15px 0px', color: '#1A1A1A' }}>{selectedTable.pg14}</Typography>

            <div>
              <Typography sx={{ fontFamily: selectedTable.font, marginTop: '65px', fontSize: '20px', fontWeight: 'bold', color: '#1A1A1A' }}>Questions</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, marginTop: '25px', marginBottom: '10px', fontWeight: 'bold', color: '#1A1A1A' }}>{selectedTable.description1}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, color: '#1A1A1A' }}>{selectedTable.d1Choice1}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, color: '#1A1A1A' }}>{selectedTable.d1Choice2}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, color: '#1A1A1A' }}>{selectedTable.d1Choice3}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, marginBottom: '10px', color: '#1A1A1A' }}>{selectedTable.d1Choice4}</Typography>
              <TextField sx={{ width: '40px', marginBottom: '10px' }} size="small" autoComplete='off' value={answer1} onChange={formHandler1} />

              <Typography sx={{ fontFamily: selectedTable.font, marginTop: '25px', fontWeight: 'bold', color: '#1A1A1A' }}>{selectedTable.description2}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, color: '#1A1A1A' }}>{selectedTable.d2Choice1}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, color: '#1A1A1A' }}>{selectedTable.d2Choice2}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, color: '#1A1A1A' }}>{selectedTable.d2Choice3}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, color: '#1A1A1A', marginBottom: '10px' }}>{selectedTable.d2Choice4}</Typography>
              {tableID > 0 && <TextField sx={{ width: '40px', marginBottom: '10px' }} size="small" autoComplete='off' value={answer2} onChange={formHandler2} />}

              <Typography sx={{ fontFamily: selectedTable.font, marginTop: '25px', fontWeight: 'bold', color: '#1A1A1A' }}>{selectedTable.description3}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, color: '#1A1A1A' }}>{selectedTable.d3Choice1}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, color: '#1A1A1A' }}>{selectedTable.d3Choice2}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, color: '#1A1A1A' }}>{selectedTable.d3Choice3}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, color: '#1A1A1A', marginBottom: '10px' }}>{selectedTable.d3Choice4}</Typography>
              {tableID > 0 && <TextField sx={{ width: '40px', marginBottom: '10px' }} size="small" autoComplete='off' value={answer3} onChange={formHandler3} />}

              <Typography sx={{ fontFamily: selectedTable.font, marginTop: '25px', fontWeight: 'bold', color: '#1A1A1A' }}>{selectedTable.description4}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, color: '#1A1A1A' }}>{selectedTable.d4Choice1}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, color: '#1A1A1A' }}>{selectedTable.d4Choice2}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, color: '#1A1A1A' }}>{selectedTable.d4Choice3}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, color: '#1A1A1A', marginBottom: '10px' }}>{selectedTable.d4Choice4}</Typography>
              {tableID > 0 && <TextField sx={{ width: '40px', marginBottom: '10px' }} size="small" autoComplete='off' value={answer4} onChange={formHandler4} />}

              <Typography sx={{ fontFamily: selectedTable.font, marginTop: '25px', fontWeight: 'bold', color: '#1A1A1A' }}>{selectedTable.description5}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, color: '#1A1A1A' }}>{selectedTable.d5Choice1}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, color: '#1A1A1A' }}>{selectedTable.d5Choice2}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, color: '#1A1A1A' }}>{selectedTable.d5Choice3}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, color: '#1A1A1A', marginBottom: '10px' }}>{selectedTable.d5Choice4}</Typography>
              {tableID > 0 && <TextField sx={{ width: '40px', marginBottom: '10px' }} size="small" autoComplete='off' value={answer5} onChange={formHandler5} />}

              <Typography sx={{ fontFamily: selectedTable.font, marginTop: '25px', fontWeight: 'bold', color: '#1A1A1A' }}>{selectedTable.description6}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, color: '#1A1A1A' }}>{selectedTable.d6Choice1}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, color: '#1A1A1A' }}>{selectedTable.d6Choice2}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, color: '#1A1A1A' }}>{selectedTable.d6Choice3}</Typography>
              <Typography sx={{ fontFamily: selectedTable.font, color: '#1A1A1A', marginBottom: '10px' }}>{selectedTable.d6Choice4}</Typography>
              {tableID > 0 && <TextField sx={{ width: '40px', marginBottom: '10px' }} size="small" autoComplete='off' value={answer6} onChange={formHandler6} />}
            </div>

            <Box sx={{ display: 'flex', justifyContent: 'center' }}>
              {tableID < 1 && <Button sx={{ marginTop: '40px' }} variant='contained' onClick={handleTestButtonClick}>Move to the experiment</Button>}
              {tableID > 0 && tableID < 3 && <Button sx={{ marginTop: '40px', mt: 1 }} variant='contained' onClick={handleTestButtonClick}>Move to the text {tableID + 1}</Button>}
              {tableID > 0 && tableID > 2 && <Button sx={{ marginTop: '40px', mt: 1 }} variant='contained' onClick={handleTestButtonClick}>Finish the test</Button>}
            </Box>

          </div>
        )}

        {!isVisible && !isEnd && tableID < 1 && webWorkerTime < 1 && <Typography sx={{ textAlign: 'center', fontFamily: 'Segoe UI', fontWeight: 'bold', marginBottom: '50px', fontSize: '20px', color: '#1A1A1A' }}>Time limit for practice test has exceeded. Press the button below to start the test number {tableID + 1}, when you are ready.</Typography>}
        ...
        {!isVisible && !isEnd && tableID > 0 && tableID < 3 && webWorkerTime < 1 && <Typography sx={{ textAlign: 'center', fontFamily: 'Segoe UI', fontWeight: 'bold', fontSize: '20px', color: '#1A1A1A', marginBottom: '50px' }}>Time limit for test {tableID} has exceeded. Press the button below to start the test number {tableID + 1}, when you are ready.</Typography>}
        ...
        {!isVisible && !isEnd && tableID === 3 && webWorkerTime < 1 && <Typography sx={{ textAlign: 'center', fontFamily: 'Segoe UI', fontWeight: 'bold', fontSize: '20px', color: '#1A1A1A', marginBottom: '50px' }}>Time limit for test {tableID} has exceeded. Press the button below to finish the experiment.</Typography>}
        {!isVisible && !isEnd && tableID === 3 && webWorkerTime > 0 && <Typography sx={{ textAlign: 'center', fontFamily: 'Segoe UI', fontWeight: 'bold', fontSize: '20px', color: '#1A1A1A', marginBottom: '50px' }}>Test {tableID} has been completed. Press the button below to finish the experiment.</Typography>}
